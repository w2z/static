...
Download_Server_Status_server(){
	cd "/tmp"
	wget -N --no-check-certificate -O master.zip "https://yunxiao-teambition-file-server.oss-cn-hangzhou.aliyuncs.com/10/1w/c6/f5127e66c04a1b3e318ee0438016b8?Expires=1596792413&OSSAccessKeyId=LTAI4FnTuFsnWwv4n43n7MqK&Signature=zWw4n9Gl8hHpi6NJ6K6dhFcp3Ps%3D&filekey=101wc6f5127e66c04a1b3e318ee0438016b8&response-content-disposition=attachment%3B+filename%3D%22ServerStatus-Toyo-master.zip%22"
...
默认为api.serverstatus.onecdn.cn，例如输入: toyoo.pw ，如果要使用本机IP，请留空直接回车"
		read -e -p "(默认: 本机IP):" server_s
		[[ -z "$server_s" ]] && server_s=""
	else
		echo -e "请输入 ServerStatus 服务端的 IP/域名[server]"
		read -e -p "(默认: api.serverstatus.onecdn.cn):" server_s
		[[ -z "$server_s" ]] && server_s="api.serverstatus.onecdn.cn"
	fi
